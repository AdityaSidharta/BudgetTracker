...
from src.saltedge.dataframe import columns
...
def connect_session(customer_id):
    r = requests.post(saltedge.connect_session_path, headers=config.headers, data=saltedge.create_session_data(customer_id))
    content = json.loads(r.content)
...
def get_connections(customer_id):
    ...
    data = content['data']
    if len(data) == 0:
        raise ValueError('The Data under `get_connection` is empty. It might be because the `connect_session` procedure'
                         ' has failed')
    elif len(data) > 1:
        raise NotImplementedError("Multiple Connections Detected. The logic for mutliple connections have yet to be implemented")
    else:
...
def get_accounts(connection_id):
    ...
    data = content['data']
    if len(data) == 0:
        raise ValueError('The Data under `get_connection` is empty. It might be '
...
def get_transactions(connection_id, account_id):
    transaction_df = pd.DataFrame(columns=columns)
    r = requests.get(saltedge.transaction_path, headers=config.headers, params={'connection_id': connection_id,
                                                                                'account_id': account_id})
    transaction_df, meta, cont = populate_transactions(r, transaction_df)
    if not cont:
        return transaction_df
    while meta['next_page'] is not None:
        r = requests.get(meta['next_page'], headers=config.headers)
        transaction_df, meta, cont = populate_transactions(r, transaction_df)
        if not cont:
            return transaction_df
    return transaction_df
