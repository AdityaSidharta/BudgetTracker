...
def get_category(input_df):
    df = input_df.copy()
    category_df = df.groupby("true_category")["amount"].sum().reset_index().sort_values("amount").reset_index(drop=True)
    ...
    if n_df <= MAX_CATEGORY:
        ...
        sum_others_df = pd.DataFrame({"true_category": ["OTHER CATEGORIES"], "amount": [others_df.amount.sum()]})
